<?php
  ...
  $repositorioProduto = new ProdutoRepositorio($pdo); 
  if (isset($_POST['editar'])) {
    ...
    if(!empty($_FILES ['imagem']['name'])){// veirificação utilizada para saber se está sendo enviado uma imagem ou será a aplicado a imagem padrão
      $nomeImagem = uniqid().$_FILES ['imagem']['name'];// tornando o nome de cada imagem unico;[]
      $produto -> setImagem($nomeImagem);
      move_uploaded_file($_FILES['imagem']['tmp_name'],$produto ->getImagemFormated());//formatando o diretorio da imagem imagem 
    }else{
      $produto -> setImagem($_POST['imagem_atual']);// com o input 'imagem_atual' eu posso recuperar o valor atual para que a imgem atual não seja modificada ao editar um produto
    }
...
<main>
  ...
  <section class="container-form">
     <form action="#" method= 'POST' enctype="multipart/form-data"><!-- enctype="multipart/form-data" nada é do que informar ao relatorio que vai se trabalar com arquivos tbm-->
